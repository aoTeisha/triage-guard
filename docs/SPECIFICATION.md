...
| Triage standard        | **ESI (Emergency Severity Index) 1-5**                    | the acuity scale. Decision point D (danger-zone vitals) is implemented deterministically; A, B and C are left to the classifier's judgment            |
...
| Acuity Classifier Agent                           | LLM (+ deterministic ESI vitals rule after it)    | model-facing payload (case_id-keyed) + history | `system_proposed_acuity` + confidence; the ESI danger-zone vitals rule may raise the proposal to emergent | LLM + ESI decision point D                 |
...
| `ACUITY_PROPOSED`                | Acuity Classifier          | `system_proposed_acuity` + confidence (may be raised by the vitals rule) | internal (proposal)       |
...
| `invoke_acuity_classifier`               | Flow step                   | no                     | yes                                  | {system_proposed_acuity, confidence}   | 7 (the LLM proposes a level from the ESI criteria, then the deterministic danger-zone vitals rule may raise it to emergent; the rule never lowers a proposal)     |
...
> - **Vitals-raised emergent:** the classifier's proposal is raised to emergent by the ESI danger-zone vitals rule before `ACUITY_PROPOSED` is emitted, then the normal gate path by gap vs nurse (`9a/9b/9c`). The raised value is the classifier's proposal; it carries no separate provenance.
...
| `system_proposed_acuity`              | Data    | enum/level                                                | Acuity Classifier (+ vitals rule)  | the classifier's **proposal** (may have been raised to emergent by the vitals rule); input to the gap, not the final value                             |
...
| Acuity write-authority           | Safety           | `acuity` is written only by (a) initial-classification resolution at the gate, (b) reassessment carrying new clinical evidence, or (c) human override. The timer alone never changes `acuity`. The vitals rule writes `system_proposed_acuity`, never `acuity`. | Datalog       |
...
>
> **The vitals rule raises a proposal, not the final acuity.** ESI decision point D can lift the classifier's proposal to emergent, but the gate still resolves that proposal against the nurse's, and a gap of 1 settles to the nurse. So the system does **not** guarantee `G(danger_zone_vitals -> acuity == emergent)`. This is the same deliberate stance the red-flag rules carried before they were retired: the clinician is the final authority, rules inform the proposal, humans decide the outcome. Both inputs are logged at 9b so the override rate can be tuned. (A design note, not an enforced invariant — it is the deliberate absence of a guarantee.)
>
> **What is implemented is ESI decision point D, not ESI.** Decision points A (life-saving intervention), B (high-risk / altered mental status / severe distress) and C (resource count) are left to the classifier's judgment and are not encoded as rules. Do not read the presence of the vitals rule as ESI conformance.
>
...
1b. The ESI decision point D danger-zone thresholds (heart rate, respiratory rate, SpO2, and the paediatric age table) must be copied from the AHRQ ESI handbook. Not yet transcribed; do not implement from memory.
...
5. `parsed_fields` schema (see Context / State variables). Intake is **structured only** as of 2026-09-13 — roughly ten dropdown / tick-box fields plus vitals; the free-text field is removed. The exact field list needs a pass with a clinician. Vitals must add **respiratory rate**, which decision point D requires and the current form does not collect.
