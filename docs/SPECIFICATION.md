...
| Acuity Classifier Agent                           | LLM (+ internal deterministic red-flag pre-check) | model-facing payload (case_id-keyed) + history | `system_proposed_acuity` + confidence; a red-flag rule can force emergent (`acuity_source = rule_forced`) | LLM + deterministic rules                  |
...
| Input Normalizer + PII filter + Sentiment/Urgency | Pre-processor                                     | routed input                                   | model-facing payload (identifiers excluded) + distress/pain score                                         | Schema-drop (identifiers) + BERT (urgency) |
...
| `invoke_acuity_classifier`               | Flow step                   | no                     | yes                                  | {system_proposed_acuity, confidence}   | 7 (agent runs its internal red-flag pre-check, then the LLM; a red-flag match returns emergent with `acuity_source=rule_forced`)                                  |
...
| `auto_resolve_acuity_upward`             | Flow step                   | yes                    | yes                                  | acuity                                 | `acuity <- more acute of {nurse, system}` (lower ESI); `acuity_source <- auto_resolved`; call `assign_order_key`; log both inputs + choice                        |
...
| `acuity`                              | Data    | enum/level                                                | Flow step (gate resolution)        | the **final resolved** acuity, written only by 9a/9b/9c; scale _(to confirm, e.g. ESI 1–5)_                                                            |
| `acuity_gap`                          | Data    | int                                                       | derived                            | absolute difference between `nurse_proposed_acuity` and `system_proposed_acuity`                                                                       |
| `acuity_source`                       | Data    | enum(system, rule_forced, auto_resolved, human_confirmed) | Flow step                          | provenance of the final `acuity`. `rule_forced` = set by the acuity agent's internal red-flag check (logged for tuning; still overridable at the gate) |
