...
| `input_rejected`           | Reject / Abort                              | invalid schema or injection (demo case 4)                | "invalid input" + the run ends (18, then 1a·rejected); a hostile submission never re-enters the pipeline                       |
...
| `redacted_payload`                    | Data    | struct                                                    | PII filter                         | model-facing; built from the allow-list in `privacy.rego`: `case_id`, `chief_complaint` (a code), `vitals`, `age_band`, `history` (condition labels; prior visits as date + acuity). **No identifiers, no prose, not the nurse's level** |
